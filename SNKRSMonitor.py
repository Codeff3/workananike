...
def scrape_site(headers, proxy):
    ...
    while anchor < 180:
        ...
        for item in output['objects']:
            items.append(item)
            logging.info(msg='Sucesso mapeando os SNKRS')
...
def discord_webhook(title, colour, slug, thumbnail):
    ...
    if title != '' and colour != '' and slug != '':
        ...
        embed["description"] = "Obrigado por usar o Juju é minha Monitors " \
                               "Se essa mensagem apareceu é porque tudo está correto" \
                               
    embed["color"] =   int(CONFIG['COLOUR'])
    embed["footer"] = {'text': 'Made by Picolino'}
    ...
    try:
        ...
        print("the octopus start his work, code {}.".format(result.status_code))
